...
- `ETHNIAFRICA_SOCIAL_PROJECTS` and `ETHNIAFRICA_SOCIAL_POSTS` are unset here. No production carousel can be claimed imported on this evidence. The owner has been asked for the configured root while independent work continues.
...
| P01   | Six uncited fact records; 24 photo file-page gaps; 26 photo license-URL gaps; social roots unset | Existing bank/illustration tests 39/39 and all 34 local photos present                                                    | Package photos hash-match existing files; Ghana live Commons page verification remains open   |
