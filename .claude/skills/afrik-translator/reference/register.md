...
| curation vocabulary     | _queue_, _candidate queue_, _research pass_, _this pass_, _research protocol_, _the protocol excludes_ / _in accordance with the protocol_, _claim-level review_, _inherited tier_, _outside the corpus_ / _out of corpus_, _coverage plan_, _wave N_, _sweep_, _Lead:_, _Research:_, _sidecar_, _translation class_, _machine provenance_ |
...
| project called an atlas | _the atlas_ as the project's own name (_the atlas does not yet document…_) — write _we_, _our project_, EthniAfrica; capitalised titles (_Atlas of the World's Languages in Danger_) and the Atlas mountains are not the project and stay                                                                                                  |
