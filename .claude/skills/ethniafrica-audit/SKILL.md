...
npm run build           # next build — the same build the production Dockerfile runs on the production VPS
```
...
**Supabase Row Level Security** — this is the data-plane gate; a table without RLS is an open door. Recette and production are both **self-hosted** stacks (ETNI-1958) that the Supabase MCP cannot see, so RLS is scored from the migration sequence (`npm run check:rls-coverage` replays it), and the live state of either is `N/A` unless read directly.
...
| admin_allowlist              | ?            | ?                 | ?            | deny-all, no policy |
| user_roles (legacy)          | ?            | ?                 | ?            | admin-only writes  |
...
2. **Validator run** — `npx tsx scripts/validateAfrikData.ts`. Parse its full output (the printed summary undercounts; read `dataset/source/afrik/logs/validation_report.json` too). The demographic bands are **warnings, never failures** (`CLAUDE.md` "Demographics", DEC-055 / REQ-170 — the earlier `docs/adr/0001` doctrine that made them hard gates is superseded and that file no longer exists):
   - **FR28** — a country's per-country population shares should sum within **[95, 105]%**.
   - **FR28-strict** — the target band **[99, 101]%**.
   - **FR28-declared** — a country that declares no split at all.
   - Report all three counts, each naming the country and the sum, and score them as **editorial debt (a warning count to burn down), not as a P0 and not as a failed check**. The reader is told at the fiche (`PeoplesSection` labels a breakdown that does not round to 100 % as estimated or incomplete), so a partial figure is a disclosed state rather than a defect.
   - Report which checks `SOFT_CHECK_NAMES` in `scripts/validateAfrikData.ts` holds advisory. Only `FR52-coverage` should be there; a check moved into that constant since the last audit is a finding, and an FR28 check found to fail the build again is a finding against `CLAUDE.md`, not against the corpus.
...
4. **Source Tier compliance** (`CLAUDE.md` "Source Tier Policy") — _nothing is forbidden, everything is labelled_. A weak source is not a defect; an unlabelled or mislabelled one is. The durable tier vocabulary is `official | referenced | unverified`; `needs_review` is the explicitly admitted transitional state for a citation nobody has ruled on yet, tracked by `NEEDS_REVIEW_RATCHET`, and is **editorial debt rather than a P0**. `source_kind` (e.g. `ai_generated`) is a separate provenance axis. Detect and classify:
   ...
   - **P0 — a `tier` value outside the admitted state vocabulary** (`official | referenced | unverified | needs_review`) shared by the corpus boundary, TypeScript state type, API and database constraint.
   - **Debt — `needs_review`.** Count it separately, verify the two-way ratchet has not increased, and report progress toward zero. Do not classify the admitted marker itself as a P0 or failed check; a missing ratchet or a count above its ceiling is a gate failure.
   ...
   - **P1 — a primary source discovered through Wikipedia whose `notes` does not record which Wikipedia language versions were crossed.** The chain is unauditable.
     ...
     ```
     Count tier states across the corpus (`official`, `referenced`, `unverified`, `needs_review`, anything else), compare them with `SourceTierState` and the latest migration that defines `sources_tier_check`, then compare the `needs_review` count with `NEEDS_REVIEW_RATCHET`.
...
| --- | ---------------------------------- | ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Security posture                   | Architecture                 | Service-role client never reachable from a browser bundle (`src/lib/supabase/admin.ts` import scan clean); the browser holds no data client (Step 3); the moderation console at `/admin` opens only to a Supabase Auth magic link sent to an address on `admin_allowlist` (migration `074`, `src/lib/supabase/moderator.ts`; `user_roles` is legacy and opens no door there), and every mutating v2 route is gated; per-request CSP nonce and the header set in `src/middleware.ts`; input validation on every external surface (v2 route param validation via `src/api/v2/utils/validation.ts`, AJV/strict schema on AFRIK ingest); API keys hashed PBKDF2-SHA256 ≥100k iterations, raw keys never stored; rate limiting real and per-tier, not dev-only in-memory; Sentry PII scrubber active with an EU DSN; RLS coverage net of drops (Step 3) with no `RLS=No` row.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 8   | AFRIK data integrity & Source Tier | Change management            | The Step 3.6 result, **scored by the literal rubric**: count the failed checks among the eight — all pass → 9–10; one failure or `N/A` → 7–8; two failures → 5–6; three or more → ≤4. A failure is a failure whatever its severity or however latent it is argued to be; do not net a failed check against a strong one or round a band up. **Any P0 from Step 3.6 check 4** (a source with no tier, an empty `sources` block or an unidentifiable source, Wikipedia cited as the source, a weak source over-tiered, AI text without its `source_kind`, a state outside the four admitted values) **caps this domain at 4** regardless of the rest — the editorial contract is the product. The admitted `needs_review` marker and the FR28 / FR28-strict / FR28-declared warning counts are debt governed by their ratchets and by DEC-055, not P0s and not caps by themselves.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 10  | Docs & runbooks                    | Emergency response           | `README`, `CLAUDE.md` and `AGENTS.md` match reality and agree with each other (no stale dual-API claims; the locale posture stated as bilingual code with publication failing closed to French-only; Next.js 16; recette and production Supabase both self-hosted since ETNI-1958, the hosted `shmrjtnfbqzceovroqjj` only a rollback until ETNI-1962); `docs/DEPLOYMENT.md` describes the actual GitHub Release → self-hosted VPS path; `docs/runbooks/restore-procedure.md` exists, the last restore drill is < 12 months old, and its next-drill-due date is not in the past; `docs/runbooks/migration-state.md` agrees with the Step 3 migration measurement; the OpenAPI document assembled from the route annotations (`apis: ["./src/app/api/v2/**/*.ts"]` in `src/lib/api/openapiV2.ts`) covers every route folder actually served under `src/app/api/v2/`, and every resource `README.md` lists is one of them; every count a doc quotes matches a measurement taken this run (a stale count is a finding even when the doctrine around it is right); the declared operational source of truth is not older than the last operational change by > 7 days; every command a doc names exists in `package.json`; named owners for recurring loops (a literal `(owner)` placeholder is a finding); this skill agrees with `CLAUDE.md`; English-docs rule respected. |
...
2. **Is the AFRIK editorial surface sound?** The domain-critical question for this project. Walk the evidence: validator verdict, FR28, FR28-strict and FR28-declared warning counts (debt, not failures), the Step 3.6 check 4 P0 counts (untiered sources, empty or unidentifiable sources, Wikipedia-as-source, over-tiered weak sources, AI text without `source_kind`, states outside the four admitted values — each must be zero), the durable-tier distribution, the separate `needs_review` count and ratchet, whether every layer can store the states, FLG/PPL/ISO referential integrity, the reader-facing register, and whether `data-integrity.yml` actually gates PRs. A site that publishes unlabelled ethnographic claims fails this question even at 10/10 everywhere else.
3. **Can a new contributor go clone → running in one session?** Walk it end to end and turn every step that has no test, no doc, or only tribal knowledge into a P1:
   ...
   - `/admin` redirects to the magic-link sign-in and admits only an address on `admin_allowlist`; the `user_roles` roles (`reader / contributor / moderator / admin / advisor`) open no door there.
...
- [ ] Step 3.6 ran: all eight AFRIK checks have a verdict in section 10, with the FR28 / FR28-strict / FR28-declared warning counts reported separately as debt, and Domain 8 was scored by counting failed checks literally.
...
2. AFRIK editorial surface sound? <FR28 warning counts (debt), Source Tier P0 count, CI gate>
...
- Any write against external services — Supabase, Sentry, Upstash, Plausible, Vercel, the self-hosted VPSs, Confluence, Jira. The audit is local and read-only; read-only `gh` and ledger reads are in scope.
