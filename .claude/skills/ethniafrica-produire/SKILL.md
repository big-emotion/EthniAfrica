...
## Scene-based video extension (2026-09-24)

When the approved storyboard is `scene-plan.json`, follow
`social/harness/SCENES.md` and invoke `ethni_montage.py <subject> --scene-plan
<path>`. Run `--validate-only`, then `--previews-only`, before the video export.
This opt-in route uses its own asset/source manifest and does not require a
carousel deck for a video-only thematic subject. The original path below stays
in place for decks and carousels.

The text-approval gate below still applies. Source hashes and alignment are
checked by the engine; no voice is generated by this path. Name-origin scripts
keep their existing category check. Other scene profiles follow the explicit
scene reference in `ethniafrica-structure`, not a fabricated name-origin type.
Review message, myth, sources and visual meaning as usual; missing gates remain
visible in the proof review. V1 exports are always watermarked under `_epreuves`,
and never mark a subject ready or change its ledger status. Resolve that output
under the registered subject's library location. An unregistered new subject
can remain in a workshop `_epreuves` folder pending proper registration; do
not invent a library category merely to run a technical proof.

Inspect the mobile previews, transitions, captions and credits, and listen to
the completed video. The engine's visual checks do not replace that review.
Do not change approved speech or loosen a guard to make a render pass.
