...
_Consequence:_ pick the right client and say which in the sub-task — `src/lib/supabase/server.ts` (SSR / server components) or `admin.ts` (service role, **server-only**, never reachable from a browser bundle); the browser never reads the corpus from Supabase, every read goes through `/api/v2`. Never widen that boundary to make a task easier. The codebase is bilingual while publication fails closed to French-only (ARCH-021, REQ-140; `SITE_LOCALE_MODE`): a sub-task that adds or changes content in one language names the sub-task that adds its counterpart, or the deferral reason (REQ-145), and consults `.claude/skills/afrik-translator/` before translating; `es`/`pt` stay closed. UI work is mobile-first at the project breakpoints — mobile 430px, tablet md 720px, desktop xl 800px.
...
- **AFRIK data** — the `validateAfrikData.ts` verdict, with the FR28 warning count and any country left outside the [95,105]% or [99,101]% band, plus the tier (`official` / `referenced` / `unverified`) and `source_kind` of every new citation.
