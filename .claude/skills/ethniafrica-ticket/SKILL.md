...
_Consequence:_ pick the right client and say which in the sub-task — `src/lib/supabase/server.ts` (SSR / server components) or `admin.ts` (service role, **server-only**, never reachable from a browser bundle); the browser never reads the corpus from Supabase, every read goes through `/api/v2`. Never widen that boundary to make a task easier. The app is bilingual, English by default (ARCH-021, REQ-140): a sub-task that adds or changes content in one language names the sub-task that adds its counterpart, or the deferral reason (REQ-145), and consults `.claude/skills/afrik-translator/` before translating; `es`/`pt` stay closed. UI work is mobile-first at the project breakpoints — mobile 430px, tablet md 720px, desktop xl 800px.
