import { QueryClient, QueryClientProvider } from "@tanstack/react-query";
import { fireEvent, render, screen, within } from "@testing-library/react";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";

import type {
  PublicFlagFilters,
  PublicFlagsPage,
} from "@/lib/supabase/queries/flags/publicFlagsPageQuery";

import { PublicFlagsQueue } from "../PublicFlagsQueue";
import { getStaticPageRoute } from "@/lib/routing";

const replaceMock = vi.fn();
const loadPublicFlagsPageMock = vi.fn();

// Mutable so one case can put the queue on its English address: the row
// permalinks follow the locale the queue is read in.
const navigation = vi.hoisted(() => ({ pathname: "/fr/signalements" }));

vi.mock("next/navigation", () => ({
  usePathname: () => navigation.pathname,
  useRouter: () => ({ replace: replaceMock }),
}));

vi.mock("next/link", () => ({
  default: ({
    children,
    href,
    ...props
  }: React.AnchorHTMLAttributes<HTMLAnchorElement> & { href: string }) => (
    <a href={href} {...props}>
      {children}
    </a>
  ),
}));

vi.mock("@/app/[lang]/signalements/actions", () => ({
  loadPublicFlagsPage: (...args: unknown[]) => loadPublicFlagsPageMock(...args),
}));

class IntersectionObserverMock {
  observe = vi.fn();
  disconnect = vi.fn();
  unobserve = vi.fn();
}

global.IntersectionObserver =
  IntersectionObserverMock as unknown as typeof IntersectionObserver;

const longReason = "a".repeat(140);
const initialPage: PublicFlagsPage = {
  items: [
    {
      id: "flag-1",
      publicSlug: "sig-2026-0001",
      kind: "missing-source",
      status: "open",
      reasonText: longReason,
      severity: "medium",
      autoGenerated: false,
      createdAt: "2026-07-23T12:00:00.000Z",
      contributorName: "",
      remediationState: null,
      remediationPublishedAt: null,
      target: {
        type: "assertion",
        id: "assertion-1",
        entityType: "people",
        entityId: "PPL_BETI",
        fieldPath: "identity.history",
        label: "Beti",
        statement: "Origine du peuple Beti",
      },
    },
  ],
  nextCursor: "next-page",
};

function renderQueue(
  page = initialPage,
  initialFilters: PublicFlagFilters = {}
) {
  const queryClient = new QueryClient({
    defaultOptions: {
      queries: { retry: false, staleTime: Infinity },
    },
  });

  return render(
    <QueryClientProvider client={queryClient}>
      <PublicFlagsQueue initialPage={page} initialFilters={initialFilters} />
    </QueryClientProvider>
  );
}

describe("PublicFlagsQueue", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    navigation.pathname = "/fr/signalements";
    window.history.replaceState({}, "", "/fr/signalements");
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-07-25T12:00:00.000Z"));
  });

  afterEach(() => {
    vi.useRealTimers();
  });

  // @req REQ-014
  it("renders a privacy-safe, mobile-first linked ledger row", () => {
    renderQueue();

    const row = screen.getByRole("article");
    const link = within(row).getByRole("link", { name: /Beti/i });
    expect(link).toHaveAttribute("href", "/fr/signalements/sig-2026-0001");
    expect(
      within(link).getByText("en cours — examen par l'équipe éditoriale")
    ).toBeInTheDocument();
    expect(within(row).getByText("Peuple")).toBeInTheDocument();
    expect(within(row).getByText("Source manquante")).toBeInTheDocument();
    expect(within(row).getByText("Assertion")).toBeInTheDocument();
    // The corpus identifier and the schema path are what the row used to end
    // on. The type, the entity label and the statement already name the target
    // in words, so the two mono lines were schema shown to the public.
    expect(within(row).queryByText("PPL_BETI")).toBeNull();
    expect(within(row).queryByText("identity.history")).toBeNull();
    expect(within(row).getByText("anonyme")).toBeInTheDocument();
    expect(within(row).getByText("il y a 2 jours")).toBeInTheDocument();

    const reason = within(row).getByTestId("flag-reason");
    expect(reason.textContent).toHaveLength(120);
    expect(reason.textContent?.endsWith("…")).toBe(true);
    expect(link.className).toContain("md:");
    expect(link.className).toContain("xl:");
  });

  // An accepted report whose correction is unpublished reads "acceptée" and
  // nothing else on the disposition pill; the queue says what became of the
  // corpus on a second, sharp-cornered mark beside it.
  // @req REQ-014
  it("marks the corpus state beside the disposition of an accepted report", () => {
    renderQueue({
      ...initialPage,
      items: [
        {
          ...initialPage.items[0],
          status: "accepted",
          remediationState: "not_started",
        },
      ],
    });

    const row = screen.getByRole("article");
    expect(within(row).getByTestId("flag-status-badge")).toHaveTextContent(
      "acceptée"
    );

    const mark = within(row).getByTestId("flag-remediation-mark");
    expect(mark).toHaveTextContent("Correction non encore publiée");
    expect(mark.className).toContain("rounded-none");
  });

  // @req REQ-014
  it("leaves the row unmarked while no remediation has been recorded", () => {
    renderQueue();

    expect(screen.queryByTestId("flag-remediation-mark")).toBeNull();
  });

  // @req REQ-014
  it("writes repeated filter values with router.replace", () => {
    renderQueue();

    fireEvent.pointerDown(screen.getByRole("button", { name: /statuts/i }), {
      button: 0,
      ctrlKey: false,
    });
    fireEvent.click(screen.getByRole("menuitemcheckbox", { name: "Ouvert" }));
    fireEvent.click(screen.getByRole("menuitemcheckbox", { name: "Accepté" }));

    expect(replaceMock).toHaveBeenLastCalledWith(
      "/fr/signalements?status=accepted&status=open",
      { scroll: false }
    );
  });

  // @req REQ-014
  it("loads continuation pages through the privacy-safe server action", async () => {
    loadPublicFlagsPageMock.mockResolvedValueOnce({
      items: [],
      nextCursor: null,
    });
    renderQueue();

    fireEvent.click(
      screen.getByRole("button", { name: "Afficher plus de signalements" })
    );

    await vi.waitFor(() => {
      expect(loadPublicFlagsPageMock).toHaveBeenCalledWith(
        expect.objectContaining({ cursor: "next-page" })
      );
    });
  });

  // @req REQ-014
  it("shows the exact empty message and resets active filters", () => {
    window.history.replaceState(
      {},
      "",
      "/fr/signalements?status=rejected&kind=broken-url&target=source"
    );
    renderQueue(
      { items: [], nextCursor: null },
      {
        statuses: ["rejected"],
        kinds: ["broken-url"],
        targetTypes: ["source"],
      }
    );

    expect(
      screen.getByText("aucun signalement ne correspond à ces filtres")
    ).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "réinitialiser" }));

    expect(replaceMock).toHaveBeenLastCalledWith("/fr/signalements", {
      scroll: false,
    });
  });

  // @req REQ-014
  it("truncates reasons at 120 Unicode code points without splitting emoji", () => {
    const emojiReason = `${"a".repeat(118)}😀😀😀`;
    renderQueue({
      ...initialPage,
      items: [{ ...initialPage.items[0], reasonText: emojiReason }],
    });

    const reason = screen.getByTestId("flag-reason").textContent ?? "";
    expect(Array.from(reason)).toHaveLength(120);
    expect(reason).toBe(`${"a".repeat(118)}😀…`);
  });

  // The row used to link every report under the French queue whatever page
  // the ledger was read on, so an English reader crossed locales on the
  // first click.
  // @req REQ-141
  it("links each report under the queue's own locale", () => {
    navigation.pathname = getStaticPageRoute("en", "reports");
    window.history.replaceState({}, "", navigation.pathname);
    renderQueue();

    expect(
      within(screen.getByRole("article")).getByRole("link", { name: /Beti/i })
    ).toHaveAttribute(
      "href",
      `${getStaticPageRoute("en", "reports")}/sig-2026-0001`
    );
  });

  // @req REQ-145
  it("renders filters, rows and relative dates in English on the English route", () => {
    navigation.pathname = getStaticPageRoute("en", "reports");
    window.history.replaceState({}, "", navigation.pathname);
    renderQueue();

    expect(
      screen.getByRole("button", { name: "Statuses" })
    ).toBeInTheDocument();
    expect(screen.getByText("People")).toBeInTheDocument();
    expect(screen.getByText("Missing source")).toBeInTheDocument();
    expect(screen.getByText("anonymous")).toBeInTheDocument();
    expect(screen.getByText("2 days ago")).toBeInTheDocument();
    expect(
      screen.getByText("in progress — editorial review")
    ).toBeInTheDocument();
  });
});
