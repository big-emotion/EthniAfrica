...
export async function generateMetadata({
  ...
  return noIndexMetadata(adminCopy[lang as Language].queue.metadataTitle);
}
...
export default async function ModerationQueuePage({
  ...
  const status = pickOption<FlagStatus>(
    firstParam(params, "statut"),
    statusOptions
  );
  const kind = pickOption<FlagKind>(firstParam(params, "type"), kindOptions);
  const entityType = pickOption(firstParam(params, "entite"), entityOptions);
  const sort = firstParam(params, "tri") === "oldest" ? "oldest" : "recent";
