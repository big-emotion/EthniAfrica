...
describe("segmentPolicy", () => {
  ...
  it("names both halves of a track that crosses a scope with a theme", () => {
    expect(quizTrackLabel("Afrique du Sud", "langues", "fr")).toBe(
      "Afrique du Sud · Langues"
    );
    expect(quizTrackLabel("Ghana", null, "fr")).toBe("Ghana");
    expect(quizTrackLabel(null, "croyances", "fr")).toBe("Croyances");
    expect(quizTrackLabel(null, null, "fr")).toBe("");
  });
  ...
  it("drops a theme the policy does not declare instead of printing it back", () => {
    expect(quizTrackLabel("Ghana", "nimportequoi", "fr")).toBe("Ghana");
    expect(quizTrackLabel(null, "nimportequoi", "fr")).toBe("");
  });
});
